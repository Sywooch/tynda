...
<div class="col-md-3">
    ...
    <?= $form->field($model, 'id_cat')->dropDownList(
        \yii\helpers\ArrayHelper::map(AfishaCat::find()->orderBy('lft')->orderBy('root')->all(), 'id', 'name')
    ) ?>
    <?= $form->field($model, 'id_place')->dropDownList(
        \yii\helpers\ArrayHelper::map(\common\models\afisha\AfishaPlace::find()->all(), 'id', 'name')
    ) ?>
    ...
    <?= $form->field($model, 'image')->widget(Cropbox::className(), [
        ...
        'previewImagesUrl' => [
            Yii::getAlias('@frt_url/img/afisha/') . $model['thumbnail']
        ],
    ]); ?>
    ...
    <div class="form-group">
        ...
        <?php
            ...
            $afisha = Afisha::find()->with('tags')->where(['id' => $model->id])->all();
            foreach ($afisha as $n) {
                // as string
    ...
    <?= $form->field($model, 'updated_at')->widget(DatePicker::classname(), [
        'name' => 'dp_2',
        'type' => DatePicker::TYPE_COMPONENT_PREPEND,
        'value' => $model->isNewRecord ? '' : $model->updated_at,
        'pluginOptions' => [
            'autoclose' => true,
            'format' => 'yyyy-mm-dd']]) ?>

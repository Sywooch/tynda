...
use common\models\realty\RealtySaleSearch;
...
class SaleController extends Controller
{
    ...
    {
        $searchModel = new RealtySaleSearch();
    ...
    private function delSession(){
        ...
        if((!empty($get['cat'])&&!empty($get['RealtySaleSearch']['cat']))){
            $ses = Yii::$app->session;
            $ses->open();
    ...
    {
        $user = Yii::$app->user->getIdentity();
        $query = RealtySale::find()->where(['id_user'=>$user->getId()]);
    ...
    public function actionChangeStatus()
    {
        ...
        if($post){
            $user = Yii::$app->user->getIdentity();
            $model = RealtySale::findOne(['id'=>$post, 'id_user'=>$user->getId()]);
            if($model->status == 1){
                $model->status = 0;
            }else{
                $model->status = 1;

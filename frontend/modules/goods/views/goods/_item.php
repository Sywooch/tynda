        ...
        <div class="container-fluid" style="<?= $color ?> padding: 1px; 10px; margin: 0px;">
            <div class="col-sm-2 side_left sm-margin-bottom-20">
                <div class="thumbnail" style="padding: 1px; margin: 15px 0px 17px 0px;">
                    <?= Html::a(Avatar::imgGoods($model['main_img'], '100%'), [Url::to('/goods/goods/view'),'cat'=>$model['alias'], 'id' => $model['id']]) ?>
                </div>
            </div>
            <div class="col-md-7 side_left">
                ...
                <h2 style="margin: 5px 0px;">
                    <?= $star ? Avatar::Star() : '&nbsp;' ?>&nbsp;
                    <?= Html::a(Html::encode($model['name']), [Url::to('/goods/goods/view'),'cat'=>$model['alias'], 'id' => $model['id']]) ?>
                </h2>
                <p><i class="small-text">Категория:&nbsp;</i><strong><?= Html::a($model['category'], [Url::to('/goods/goods/index'),'cat'=>$model['alias']]); ?></strong></p>

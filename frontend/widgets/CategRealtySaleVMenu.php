...
class CategRealtySaleVMenu extends Widget
{
    public function run($level = 0){
        $cat = RealtyCat::find()
            ->select(['id', 'root', 'lft', 'rgt', 'lvl', 'name', 'icon', 'alias', 'area_home', 'area_land', 'floor', 'floor_home','comfort'])
        ...
        echo '<h3 style="margin: 0; color: #fff; font-size: 1.2em;"><i class="fa fa-building"></i>&nbsp;&nbsp;Продажа недвижимости</h3>';
        echo '</div>';
        echo '<div  style="margin: 0px 0px 2px 0px; width: 100%; border: solid 1px #780000;">';
        echo Html::a('Продажа', [Url::to('/realty/sale/index')], ['class' => 'btn-u btn-u-primary', 'style'=>'width:50%; text-align: center; font-size: 11px; text-transform: uppercase; font-weight: 400;']);
        echo Html::a('Аренда', [Url::to('/realty/rent/index')], ['class' => 'btn-u btn-u-default', 'style'=>'width:50%; text-align: center; font-size: 11px; text-transform: uppercase; font-weight: 400;']);
        echo '</div>';
        echo Html::a('<i class="fa fa-plus"></i>&nbsp;&nbsp;Подать объявление', ['/realty/sale/create'], ['class' => 'btn-u btn-u-orange cat-button','style'=>'padding: 5px 7px 5px 7px; text-align:center; font-size:15px; width:100%;']);

...
/**
 * This is the model class for table "realty_rent".
 *
 * @property string $id
...
class RealtyRent extends \yii\db\ActiveRecord
{
    ...
    public function upload()
    {
        if (count($this->images) > 0) {
            $path = Yii::getAlias('@frt_dir/img/realty_rent/');
            $rand = time();
            $count_img = RealtyRentImg::find()->where(['id_ads' => $this->id, 'id_user' => $this->id_user])->count();
            ...
            if ($count <= Arrays::IMG_COUNT) {
                ...
                $db = Yii::$app->db;
                $db->createCommand()->batchInsert('realty_rent_img',['id_ads','id_user','img'],$img_array)->execute();
                return true;
            } else {
    ...
    {
        $f_img = RealtyRentImg::find()->where(['id_ads'=>$id,'id_user'=>$id_user])->asArray()->all();
        $path = Yii::getAlias('@frt_dir/img/realty_rent/');
        if(count($f_img)>0){
            ...
            if(!RealtyRentImg::deleteAll(['id_ads'=>$id,'id_user'=>$id_user])){
                return false;
            }
        }return true;
    }
    ...
    {
        $img = RealtyRentImg::findOne(['id'=>$id,'id_ads'=>$id_ads,'id_user'=>$id_user]);
        $path = Yii::getAlias('@frt_dir/img/realty_rent/');
        if($img !== null){
            $file = $path . $img['img'];
            if(is_file($file)){
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($this->image->tempName != '') {
            ...
            //delete old images
            $oldImages = FileHelper::findFiles(Yii::getAlias('@frt_dir/img/realty_rent/'), [
                'only' => [
                    $this->id . '.*',
                    'main_' . $this->id . '.*',
                ],
            ...
            $pathThumbImage = Yii::getAlias('@frt_dir/img/realty_rent/') . $imgName;

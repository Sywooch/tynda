...
/**
 * RealtySaleSearch represents the model behind the search form about `common\models\realty\RealtySale`.
 */
class RealtySaleSearch extends VRealtySale
{
    ...
    public function search($params)
    {
        ...
        if(isset($cur_cat)){
            ...
            if(is_array($id_cat)){
                ...
                $query = VRealtySale::find()->where("id_cat IN ($id_str)");
            }else{
                $query = VRealtySale::find()->where(['id_cat' => $id_cat]);
            }
        }else{
            $query = VRealtySale::find();
        }

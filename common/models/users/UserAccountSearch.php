...
class UserAccountSearch extends UserAccount
{
    ...
    {
        if($in_out == 'in'){
            $query = UserAccount::find()->with('idUser')->andWhere(['id_user'=>\Yii::$app->user->id])->andWhere(['IS NOT','pay_in', NULL])->orderBy('id DESC');
        }else{
            $query = UserAccount::find()->with('idUser')->andWhere(['id_user'=>\Yii::$app->user->id])->andWhere(['IS NOT','pay_out', NULL])->orderBy('id DESC');
        }
